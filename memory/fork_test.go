...
func TestForkJournalSharesParentPrefix(t *testing.T) {
	...
	for i, name := range []string{"a", "b", "c"} {
		if err := parent.Store(i, dispatcher.Call{Name: name}, dispatcher.Result([]byte(name))); err != nil {
	...
	if err := child.Store(2, dispatcher.Call{Name: "c2"}, dispatcher.Result([]byte("c2"))); err != nil {
...
func TestForkJournalChainsThroughRevisions(t *testing.T) {
	...
	for i, name := range []string{"a", "b", "c", "d"} {
		if err := j1.Store(i, dispatcher.Call{Name: name}, dispatcher.Result(nil)); err != nil {
	...
	if err := j2.Store(3, dispatcher.Call{Name: "d2"}, dispatcher.Result(nil)); err != nil {
